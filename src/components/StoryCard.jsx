...
function StoryCard({ id, data, selected }) {
  ...
  const duplicateCard = useCallback(() => {
    try {
      ...
      const newNode = {
        ...
        data: { ...(original?.data || {}), text, title, tags },
      };
  ...
  }, [getNodes, id, setNodes, text, title]);
  ...
  const saveTagsFromDialog = useCallback(() => {
    setNodes((nds) => nds.map((node) =>
      node.id === id ? { ...node, data: { ...node.data, tags: tempTags } } : node
    ));
  ...
  const updateDailyMessageCount = useCallback(async () => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        const messagesData = userData.dailyMessages || {};
        currentCount = messagesData[today] || 0;
      }
  ...
  // Load daily message count on component mount
  useEffect(() => {
    const loadDailyMessageCount = async () => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          const userData = userDoc.data();
          const messagesData = userData.dailyMessages || {};
          setDailyMessageCount(messagesData[today] || 0);
        }
      } catch (error) {
