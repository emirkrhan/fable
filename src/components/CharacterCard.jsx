...
function CharacterCard({ id, data, selected }) {
  ...
  const duplicateCard = useCallback(() => {
    try {
      ...
      const maxId = nodes.map((n) => parseInt(n.id, 10)).filter((n) => !Number.isNaN(n)).reduce((acc, n) => Math.max(acc, n), 0);
      const newId = (maxId || 0) + 1;
      const newPosition = original?.position ? { x: original.position.x + 40, y: original.position.y + 40 } : { x: 120, y: 120 };
      const newNode = {
        id: String(newId),
        type: 'characterCard',
  ...
  }, [getNodes, id, setNodes, profileUrl, firstName, lastName, synopsis]);
